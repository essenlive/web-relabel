...
declare const global: typeof globalThis & { __sqlite_db?: Database.Database };
...
function transformRow(row: Record<string, unknown>, tableName: string): Record<string, unknown> {
    ...
    const result: Record<string, unknown> = { id: row.airtable_id };
    ...
    for (const [key, value] of Object.entries(row)) {
        ...
        if (meta.attachmentFields.includes(key)) {
            if (value) {
                const objects = JSON.parse(value as string) as Array<{ localPath?: string; url: string }>;
...
function findMany(tableName: string, args?: FindManyArgs): Record<string, unknown>[] {
    const meta = tableMetadata[tableName] || { arrayFields: [], attachmentFields: [], booleanFields: [] };
...
interface PrismaClient {
    community: PrismaModel;
    project: PrismaModel;
    structure: PrismaModel;
    [key: string]: PrismaModel | undefined;
}
...
const prisma = new Proxy({} as PrismaClient, {
    get(_target, modelName: string) {
        ...
        return { findMany: (args?: FindManyArgs) => findMany(tableName, args) };
    }
});
...
export function filter<T extends Record<string, unknown>>(data: T[], filterObj: Record<string, unknown>): T[] {
    if (!Object.keys(filterObj).length) return data;

    const singleFilter = Object.entries(filterObj)[0]
...
export function createApi(type: string, fields: Array<{ fields: Record<string, unknown> }>): Promise<unknown> {
    return new Promise((resolve, reject) => {
        base(type).create(fields as any, { typecast: true }, function (err: Error | undefined, records: unknown) {
            if (err) { reject(err); }
            resolve(records);
        });
    });
...
export async function manageImages(localPath: string, name: string, id: number): Promise<string> {
